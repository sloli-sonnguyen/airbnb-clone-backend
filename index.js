...
const getRooms = () => {
  app.get('/rooms', (req, res) => {
    const { query } = req;
    if(query.city || query.home_type){
        
        db.query(
            `
                  SELECT room.id, room.price, room.home_type, room.total_bedrooms, room.total_bathrooms,
                          room.city, room.has_tv, room.has_air_con, room.has_heating, room.has_internet,
                          room.state, url, room_id FROM room,  images
                          where room.${Object.keys(query)[0]} = "${query[Object.keys(query)[0]]}" 
                          and room.id = images.room_id group by room.id;`,
        ...
        db.query(
            `
                  SELECT room.id, room.price, room.home_type, room.total_bedrooms, room.total_bathrooms,
                          room.city, room.has_tv, room.has_air_con, room.has_heating, room.has_internet,
                          room.state, url, room_id FROM room, images
                          where room.id = images.room_id group by room.id;`,
            function (err, result, fields) {
              if (err) throw err;
              res.json(result);
            }
...
const getImageUrlByRoomId = () => {
  app.get('/images/by-roomId/:id', (req, res) => {
    const { id } = req.params;
    db.query(
      `select url as url_image from room, images where room.id = images.room_id and room.id = ${id};`,
      function (err, imageUrls, fields) {
        if (err) throw err;
        res.json(imageUrls);
      }
    );
...
const getHometype = () => {
  app.get('/room-hometypes', (req, res) => {
    db.query(
      `select room.home_type, count(*) as counted from room group by home_type order by counted desc;`,
      function (err, hometypes, fields) {
        if (err) throw err;
        res.json(hometypes);
      }
    );
...
const getHostById = () => {
  app.get('/hosts/:id', (req, res) => {
    const { id } = req.params;
    db.query(
      `SELECT * FROM host where host.id = ${id}`,
      function (err, result, fields) {
        if (err) throw err;
        res.json(result);
      }
...
const getHighRatingHost = () => {
  app.get('/top-rating-host', (req, res) => {
    db.query(
      `select * from host where host.rating = (select max(rating) from host);`,
      function (err, result, fields) {
        if (err) throw err;
        res.json(result);
      }
...
const getPurchaseById = () => {
  app.get('/purchases/:id', (req, res) => {
    const { id } = req.params;
    db.query(
      `SELECT * FROM reservations where reservations.id = ${id}`,
      function (err, result, fields) {
        if (err) throw err;
        res.json(result);
      }
